// @flow

import color from 'color';
import { Platform, Dimensions, PixelRatio } from 'react-native';
import I18n from '../../src/i18n';

import { PLATFORM } from './commonColor';
import {Colors} from "../../src/themes/variables";

const deviceHeight = Dimensions.get('window').height;
const deviceWidth = Dimensions.get('window').width;
const platform = Platform.OS;
const platformStyle = undefined;
const isIphoneX =
  platform === PLATFORM.IOS &&
  (deviceHeight === 812 ||
    deviceWidth === 812 ||
    deviceHeight === 896 ||
    deviceWidth === 896);

export default Colors => {
  return (
    {
      platformStyle,
      platform,
      bg_1: Colors.bg_1,
      bg_2: Colors.bg_2,
      bg_3: Colors.bg_3,
      backdrop: Colors.backdrop,
      text_1: Colors.text_1,
      text_2: Colors.text_2,
      text_3: Colors.text_3,
      statusBarContent: Colors.statusBarContent,

      // Accordion
      accordionBorderColor: '#d3d3d3',
      accordionContentPadding: 10,
      accordionIconFontSize: 18,
      contentStyle: '#f5f4f5',
      expandedIconStyle: '#000',
      headerStyle: '#edebed',
      iconStyle: '#000',

      // ActionSheet
      elevation: 4,
      containerTouchableBackgroundColor: 'rgba(0,0,0,0.4)',
      innerTouchableBackgroundColor: '#fff',
      listItemHeight: 50,
      listItemBorderColor: 'transparent',
      marginHorizontal: -15,
      marginLeft: 14,
      marginTop: 15,
      minHeight: 56,
      padding: 15,
      touchableTextColor: '#757575',

      // Android
      androidRipple: true,
      androidRippleColor: 'rgba(256, 256, 256, 0.3)',
      androidRippleColorDark: 'rgba(0, 0, 0, 0.15)',
      buttonUppercaseAndroidText: true,

      // Badge
      badgeBg: Colors.bg_2,
      badgeColor: '#fff',
      badgePadding: platform === PLATFORM.IOS ? 3 : 0,

      // Button
      buttonFontFamily: I18n.fontFamily.buttonBold,
      buttonDisabledBg: '#b5b5b5',
      buttonPadding: 6,
      buttonDefaultActiveOpacity: 0.5,
      buttonDefaultFlex: 1,
      buttonDefaultBorderRadius: 2,
      buttonDefaultBorderWidth: 1,
      buttonAddListBg: Colors.buttonAddListBg,
      get buttonPrimaryBg() {
        return this.brandPrimary;
      },
      get buttonPrimaryColor() {
        return this.inverseTextColor;
      },
      get buttonWhiteBg() {
        return Colors.bg_1;
      },
      get buttonWhiteColor() {
        return this.brandDark;
      },
      get buttonInfoBg() {
        return this.brandInfo;
      },
      get buttonInfoColor() {
        return this.inverseTextColor;
      },
      get buttonSuccessBg() {
        return this.brandSuccess;
      },
      get buttonSuccessColor() {
        return this.inverseTextColor;
      },
      get buttonDangerBg() {
        return this.brandDanger;
      },
      get buttonDangerColor() {
        return this.inverseTextColor;
      },
      get buttonWarningBg() {
        return this.brandWarning;
      },
      get buttonWarningColor() {
        return this.inverseTextColor;
      },
      get buttonTextSize() {
        // return platform === PLATFORM.IOS
        //   ? this.fontSizeBase * 1.1
        //   : this.fontSizeBase - 1;
        return this.fontSizeBase - 1;
      },
      get buttonTextSizeLarge() {
        return this.fontSizeBase * 1.5;
      },
      get buttonTextSizeSmall() {
        return this.fontSizeBase * 0.8;
      },
      get borderRadiusLarge() {
        return this.fontSizeBase * 3.8;
      },
      get iconSizeLarge() {
        return this.iconFontSize * 1.5;
      },
      get iconSizeSmall() {
        return this.iconFontSize * 0.6;
      },

      // Card
      cardDefaultBg: Colors.bg_1,
      cardBorderColor: Colors.bg_3,
      cardBorderRadius: 3,
      curveCardBorderRadius: 8,
      cardItemPadding: platform === PLATFORM.IOS ? 10 : 12,

      //  ListCard
      listCardBg: Colors.bg_1,

      //  Board
      defaultBoardBg: Colors.defaultBoardBg,
      boardHeaderColor: Colors.boardHeaderColor,
      boardHeaderTitle: Colors.text_3,

      // BoardList
      boardListBg: Colors.listBg,

      // CheckBox
      CheckboxRadius: platform === PLATFORM.IOS ? 13 : 0,
    CheckboxBorderWidth: platform === PLATFORM.IOS ? 1 : 2,
    CheckboxPaddingLeft: platform === PLATFORM.IOS ? 4 : 2,
    CheckboxPaddingBottom: platform === PLATFORM.IOS ? 0 : 5,
    CheckboxIconSize: platform === PLATFORM.IOS ? 21 : 16,
    CheckboxIconMarginTop: platform === PLATFORM.IOS ? undefined : 1,
    CheckboxFontSize: platform === PLATFORM.IOS ? 23 / 0.9 : 17,
    checkboxBgColor: '#039BE5',
    checkboxSize: 20,
    checkboxTickColor: '#fff',
    checkboxDefaultColor: 'transparent',
    checkboxTextShadowRadius: 0,

    // Color
    brandPrimary: '#1775ef',
    brandInfo: '#62B1F6',
    brandSuccess: '#5cb85c',
    brandDanger: '#d9534f',
    brandWarning: '#f0ad4e',
    // brandDark: '#778ca2',
    // brandLight: '#f2f4f6',
    brandDark: Colors.text_1,
    brandGray: Colors.bg_2,
    brandLight: Colors.bg_1,

    // Container
    containerBgColor: Colors.bg_2,
    containerLighterBgColor: Colors.bg_1,

    // Date Picker
    datePickerFlex: 1,
    datePickerPadding: 10,
    datePickerTextColor: '#000',
    datePickerBg: 'transparent',

    // FAB
    fabBackgroundColor: 'blue',
    fabBorderRadius: 28,
    fabBottom: 0,
    fabButtonBorderRadius: 20,
    fabButtonHeight: 40,
    fabButtonLeft: 7,
    fabButtonMarginBottom: 10,
    fabContainerBottom: 20,
    fabDefaultPosition: 20,
    fabElevation: 4,
    fabIconColor: '#fff',
    fabIconSize: 24,
    fabShadowColor: '#000',
    fabShadowOffsetHeight: 2,
    fabShadowOffsetWidth: 0,
    fabShadowOpacity: 0.4,
    fabShadowRadius: 2,
    fabWidth: 56,

    // Font
    DefaultFontSize: 19,
    fontFamily: I18n.fontFamily.main,
    lightFontFamily: I18n.fontFamily.mainLight,
    boldFontFamily: I18n.fontFamily.mainBold,
    fontSizeBase: 15,
    get fontSizeH1() {
    return this.fontSizeBase * 1.8;
  },
  get fontSizeH2() {
    return this.fontSizeBase * 1.6;
  },
  get fontSizeH3() {
    return this.fontSizeBase * 1.4;
  },

  // Footer
  footerHeight: 55,
    footerDefaultBg: Colors.footerBg,
    footerPaddingBottom: 0,

    // FooterTab
    tabBarTextColor: Colors.footerText,
    tabBarTextSize: platform === PLATFORM.IOS ? 14 : 11,
    activeTab: platform === PLATFORM.IOS ? '#007aff' : '#fff',
    sTabBarActiveTextColor: '#007aff',
    tabBarActiveTextColor: Colors.primary,
    tabActiveBgColor: '#1775ef',

    // Header
    toolbarBtnColor: platform === PLATFORM.IOS ? '#007aff' : '#fff',
    toolbarDefaultBg: Colors.bg_1,
    toolbarHeight: platform === PLATFORM.IOS ? 64 : 56,
    toolbarSearchIconSize: platform === PLATFORM.IOS ? 20 : 23,
    toolbarInputColor: '#f2f4f6',
    searchBarHeight: platform === PLATFORM.IOS ? 30 : 40,
    searchBarInputHeight: platform === PLATFORM.IOS ? 30 : 50,
    toolbarBtnTextColor: '#007aff',
    toolbarDefaultBorder: platform === PLATFORM.IOS ? '#a7a6ab' : '#3F51B5',
    iosStatusbar: platform === PLATFORM.IOS ? 'dark-content' : 'light-content',
    get statusBarColor() {
    return color(this.toolbarDefaultBg)
      .darken(0.2)
      .hex();
  },
  get darkenHeader() {
    return color(this.tabBgColor)
      .darken(0.03)
      .hex();
  },

  // Icon
  iconFamily: 'Ionicons',
    iconFontSize: platform === PLATFORM.IOS ? 30 : 28,
    iconHeaderSize: platform === PLATFORM.IOS ? 33 : 24,

    // InputGroup
    inputFontSize: 17,
    inputBorderColor: '#D9D5DC',
    inputTextColor: Colors.text_2,
    inputBackgroundColor: Colors.bg_3,
    inputLightBackgroundColor: Colors.bg_1,
    inputSuccessBorderColor: '#2b8339',
    inputErrorBorderColor: '#ed2f2f',
    inputHeightBase: 50,
    get inputColor() {
    return this.textColor;
  },
  get inputColorPlaceholder() {
    return '#575757';
  },

  // Line Height
    buttonLineHeight: 19,
    lineHeightH1: 34,
    lineHeightH2: 32,
    lineHeightH3: 30,
    lineHeight: platform === PLATFORM.IOS ? 20 : 24,
    listItemSelected: '#1775ef',
    listItemDisabled: '#808080',

    // List
    listContainer: Colors.bg_1,
    listBg: 'transparent',
    listBorderColor: '#c9c9c9',
    listDividerBg: Colors.bg_2,
    listBtnUnderlayColor: '#DDD',
    listItemPadding: platform === PLATFORM.IOS ? 10 : 12,
    listNoteColor: '#808080',
    listNoteSize: 13,

    // Progress Bar
    defaultProgressColor: '#E4202D',
    inverseProgressColor: '#1A191B',

    // Radio Button
    radioBtnSize: platform === PLATFORM.IOS ? 25 : 23,
    radioSelectedColorAndroid: '#3F51B5',
    radioBtnLineHeight: platform === PLATFORM.IOS ? 29 : 24,
    get radioColor() {
    return this.brandPrimary;
  },

  // Segment
  // segmentBackgroundColor: platform === PLATFORM.IOS ? '#F8F8F8' : '#3F51B5',
  // segmentActiveBackgroundColor: platform === PLATFORM.IOS ? '#007aff' : '#fff',
  // segmentTextColor: platform === PLATFORM.IOS ? '#007aff' : '#fff',
  // segmentActiveTextColor: platform === PLATFORM.IOS ? '#fff' : '#3F51B5',
  // segmentBorderColor: platform === PLATFORM.IOS ? '#007aff' : '#fff',
  // segmentBorderColorMain: platform === PLATFORM.IOS ? '#a7a6ab' : '#3F51B5',
  segmentBorderColorMain: '#a7a6ab',
    segmentBackgroundColor: Colors.bg_1,
    segmentActiveBackgroundColor: '#007aff',
    segmentTextColor: '#007aff',
    segmentActiveTextColor: '#fff',
    segmentBorderColor: '#007aff',

    // Spinner
    defaultSpinnerColor: '#45D56E',
    inverseSpinnerColor: '#1A191B',

    // Tab
    tabBarDisabledTextColor: '#BDBDBD',
    tabDefaultBg: platform === PLATFORM.IOS ? '#F8F8F8' : '#3F51B5',
    topTabBarTextColor: platform === PLATFORM.IOS ? '#6b6b6b' : '#b3c7f9',
    topTabBarActiveTextColor: platform === PLATFORM.IOS ? '#007aff' : '#fff',
    topTabBarBorderColor: platform === PLATFORM.IOS ? '#a7a6ab' : '#fff',
    topTabBarActiveBorderColor: platform === PLATFORM.IOS ? '#007aff' : '#fff',

    // Tabs
    tabBgColor: '#F8F8F8',
    tabFontSize: 15,

    // Text
    textColor: Colors.text_1,
    noteColor: Colors.text_2,
    inverseTextColor: '#fff',
    noteFontSize: 17,
    get defaultTextColor() {
    return this.textColor;
  },

  // Title
  titleFontFamily: I18n.fontFamily.title,
    titleFontSize: platform === PLATFORM.IOS ? 17 : 19,
    subTitleFontSize: platform === PLATFORM.IOS ? 11 : 14,
    subtitleColor: platform === PLATFORM.IOS ? '#8e8e93' : '#FFF',
    titleFontColor: Colors.text_1,

    // Thumbnail
    thumbnailBackground: Colors.memberBg,

    // Other
    borderRadiusBase: 5,
    borderWidth: 1 / PixelRatio.getPixelSizeForLayoutSize(1),
    contentPadding: 10,
    dropdownLinkColor: '#414142',
    inputLineHeight: 24,
    deviceWidth,
    deviceHeight,
    isIphoneX,
    inputGroupRoundedBorderRadius: 30,

    // iPhoneX SafeArea
    Inset: {
    portrait: {
      topInset: 24,
        leftInset: 0,
        rightInset: 0,
        bottomInset: 34
    },
    landscape: {
      topInset: 0,
        leftInset: 44,
        rightInset: 44,
        bottomInset: 21
    }
  }
}
);
};
